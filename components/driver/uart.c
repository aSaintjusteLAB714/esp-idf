...
#define UART_INTR_CONFIG_FLAG ((UART_INTR_RXFIFO_FULL) | (UART_INTR_RXFIFO_TOUT) | (UART_INTR_RXFIFO_OVF) | (UART_INTR_BRK_DET) | (UART_INTR_PARITY_ERR))
...
esp_err_t UART_ISR_ATTR uart_enable_intr_mask_in_isr(uart_port_t uart_num, uint32_t enable_mask)
{
    ESP_RETURN_ON_FALSE((uart_num < UART_NUM_MAX), ESP_FAIL, UART_TAG, "uart_num error");
...
esp_err_t UART_ISR_ATTR uart_disable_intr_mask_in_isr(uart_port_t uart_num, uint32_t disable_mask)
{
    ESP_RETURN_ON_FALSE((uart_num < UART_NUM_MAX), ESP_FAIL, UART_TAG, "uart_num error");
...
static void UART_ISR_ATTR uart_rx_intr_handler_default(void *param)
{
    ...
    while (1)
    {
        ...
        else if ((uart_intr_status & UART_INTR_RXFIFO_TOUT) || (uart_intr_status & UART_INTR_RXFIFO_FULL) || (uart_intr_status & UART_INTR_CMD_CHAR_DET))
        {
            if (pat_flg == 1)
            {
        ...
        else if ((uart_intr_status & UART_INTR_RS485_PARITY_ERR) || (uart_intr_status & UART_INTR_RS485_FRM_ERR) || (uart_intr_status & UART_INTR_RS485_CLASH))
        {
...
esp_err_t uart_set_mode(uart_port_t uart_num, uart_mode_t mode)
{
    ...
    if (mode == UART_MODE_RS485_COLLISION_DETECT)
    {
        ...
        uart_hal_ena_intr_mask(&(uart_context[uart_num].hal), UART_INTR_RXFIFO_TOUT | UART_INTR_RXFIFO_FULL | UART_INTR_RS485_CLASH | UART_INTR_RS485_FRM_ERR | UART_INTR_RS485_PARITY_ERR);
    }
