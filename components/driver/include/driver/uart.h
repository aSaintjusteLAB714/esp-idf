...
#define UART_BITRATE_MAX SOC_UART_BITRATE_MAX ///< Maximum configurable bitrate
  ...
  /**
   * @brief Set UART interrupt enable from isr
   *
   * @param uart_num     UART port number, the max port number is (UART_NUM_MAX -1).
   ...
   *     - ESP_FAIL Parameter error
   */
  esp_err_t uart_enable_intr_mask_in_isr(uart_port_t uart_num, uint32_t enable_mask);

  /**
   * @brief Clear UART interrupt enable bits from isr
   *
   * @param uart_num      UART port number, the max port number is (UART_NUM_MAX -1).
   ...
   *     - ESP_FAIL Parameter error
   */
  esp_err_t uart_disable_intr_mask_in_isr(uart_port_t uart_num, uint32_t disable_mask);
